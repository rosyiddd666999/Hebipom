...
import 'package:hebipom/futures/data/model/isar_habit.dart';
...
class HabitRepoImpl implements HabitRepo {
  ...
  Future<void> _processHabitReset(HabitIsar habit) async {
    ...
    if (HabitUtils.shouldResetCompleted(
      ...
      // PENTING: Tambahkan tanggal kemarin ke completedDates jika habit completed
      if (habit.isCompleted && habit.lastCompletedDate != null) {
        final yesterday = HabitUtils.normalizeToDay(habit.lastCompletedDate!);
        
        // Cek apakah tanggal kemarin sudah ada di list (hindari duplikat)
        final alreadyExists = habit.completedDates.any((date) {
          final normalized = HabitUtils.normalizeToDay(date);
          return normalized.isAtSameMomentAs(yesterday);
        });
        
        if (!alreadyExists) {
          habit.completedDates = [...habit.completedDates, yesterday];
        }
      }
